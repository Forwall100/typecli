...
def load_word_list(language_name):
    ...
    if "words" not in data:
        raise ValueError(f"Invalid format in {language_name}.json, 'words' field not found.")
...
def draw_text(stdscr, target, current, current_pos, wpm=0, time_left=0):
    ...
    draw_centered_text(stdscr, screen_height // 2 - 5, f"WPM: {wpm} | Time left: {time_left:.1f}s")
...
def show_results(stdscr, wpm, accuracy):
    ...
    draw_centered_text(stdscr, screen_height // 2 + 4, "Press 'r' to restart or 'q' to quit")
...
def run_wpm_test(stdscr, word_list, test_duration):
    ...
    while True:
        ...
        accuracy = sum(1 for c, t in zip(current_text, target_text) if c == t) / len(current_text) * 100 if current_text else 0
